...
import {searchCourse} from '../../queries/queries';
...
var randomColor = require('randomcolor');
 class SearchCourse extends Component {
    ...
    handleNext = (e) => {
      ...
      axios.get( ROOT_URL + `/course/search?page=${currentPage}&limit=${this.state.limit}&searchTerm=${this.state.searchterm}`, {
    ...
    if(res.data.searchCourse.result) {
        this.setState({
            courses : res.data.searchCourse.courseDetails
        });
    }
   ...
   axios.post( ROOT_URL + `/searchCourses`, data, {
    headers: {"Authorization" : `Bearer ${localStorage.getItem('token')}`}
} ) 
            .then((response) => {
